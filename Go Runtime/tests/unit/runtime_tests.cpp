...
int test_executes_submitted_tasks() {
  ...
  const bool completed =
      cv.wait_for(lock, 3s, [&] { return done.load() == kTaskCount; });
  runtime.shutdown();
  if (!completed) {
    std::cerr
        << "Test failed: Not all tasks completed within the expected time."
        << std::endl;
    return 1;
  }
  return 0;
...
int test_submit_rejected_after_shutdown() {
  ...
  try {
    ...
    std::cerr << "test submit rejected after shutdown: Caught unexpected "
                 "exception type."
              << std::endl;
    return 1;
  }
  std::cerr << "Test failed: Task submission should have been rejected after "
               "shutdown."
            << std::endl;
  return 1;
}
